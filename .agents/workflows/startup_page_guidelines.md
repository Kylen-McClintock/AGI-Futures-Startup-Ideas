...
- **Startup Descriptor**: Every startup idea MUST display its short, 4-5 word descriptor (the same phrase used as the `scoreTitle` in the homepage link card, e.g., "Make Identifying Corruption Profitable") prominently in the Hero section, typically as an `<h2>` subtitle directly under the main specific `<h1>` title.
...
- **Togglable ICP Component**: The Ideal Customer Profile (ICP) section MUST feature a togglable component (e.g., `<TogglableICP />`) that allows users to click through different customer personas. Each persona must load a visually distinct, high-fidelity custom image tailored to that use case, alongside a descriptive paragraph.
...
- **Forecasting (AUTOMATIC)**: You do NOT need to manually import or render the `<CategoryFrontierForecast />` component in your `page-client.tsx`. It is globally injected via the root layout and will automatically append itself to the bottom of your page, perfectly matching whatever `--primary`, `--secondary`, and `--tertiary` hex color CSS variables you set on your `<main>` tag.
    *   **CRITICAL DATA STEP:** While the UI injection is automatic, you MUST add a new dictionary entry to the `forecastDatabase` object inside `src/data/forecasts.ts` for the new route slug. Assign an appropriate curve (`SaaSMarketCurve`, `DeepTechCurve`, or `AIEmergenceCurve`) and write a custom, highly accurate `aiRationale` string (1-3 sentences) analyzing the market dynamics, bottlenecks, and scaling potential of the new idea. **The rationale must explicitly reference the "AGI Futures forecaster model" (or similar phrasing) as the entity making the prediction.**
