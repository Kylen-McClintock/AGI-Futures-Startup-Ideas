...
export function ScoreCard({ title, score, maxScore = 100, summary, details, type }: ScoreCardProps) {
    ...
    const bgClass = "from-[var(--secondary)]/20";
    const borderClass = "border-[var(--primary)]/20";
    const Icon = type === "moat" ? Shield : Crosshair;

    return (
        <div className={`glass-panel p-6 sm:p-8 rounded-3xl border ${borderClass} relative overflow-hidden bg-white/5`}>
            {/* Background gradient */}
            <div className={`absolute inset-0 bg-gradient-to-br ${bgClass} to-transparent z-0 pointer-events-none`} />
            ...
            <div className="relative z-10 w-full">
                ...
                <AnimatePresence>
                    {isExpanded && (
                        ...
                        >
                            <div className="pt-6 border-t border-[var(--primary)]/10 text-white/70 space-y-4">
                                {details}
                            </div>
                        </motion.div>
                    )}
                ...
                <button
                    ...
                    className={`mt-4 w-full py-4 flex items-center justify-center text-sm font-mono uppercase tracking-widest transition-colors rounded-xl border border-white/5 hover:border-[var(--primary)]/20 hover:bg-white/5 ${colorClass}`}
                >
...
export function RiskItem({ level, title, description, mitigation }: { level: string, title: string, description: React.ReactNode, mitigation: React.ReactNode }) {
    ...
    if (level === "High") levelClass = "text-orange-400";
    ...
    return (
        <div className="mb-6 last:mb-0">
            ...
            {mitigation && (
                <div className="bg-white/5 border border-[var(--primary)]/10 rounded-xl p-4">
