...
export function SkillTransition() {
    ...
    // Stage 1: Initial Skills Visibility (Fades out as we scroll down)
    const skillsOpacity = useTransform(smoothProgress, [0, 0.4], [1, 0]);
    const skillsY = useTransform(smoothProgress, [0, 0.4], [0, -50]);
    const skillsScale = useTransform(smoothProgress, [0, 0.4], [1, 0.95]);

    // Stage 2: The Transition Graphic (Lines converging/diverging)
    const linesOpacity = useTransform(smoothProgress, [0.2, 0.5, 0.8], [0, 1, 0]);
    const linesScale = useTransform(smoothProgress, [0.2, 0.5, 0.8], [0.8, 1, 0.8]);

    // Stage 3: Occupations Visibility (Fades in as we reach the bottom)
    const occupationsOpacity = useTransform(smoothProgress, [0.6, 1], [0, 1]);
    const occupationsY = useTransform(smoothProgress, [0.6, 1], [50, 0]);
    const occupationsScale = useTransform(smoothProgress, [0.6, 1], [0.95, 1]);
