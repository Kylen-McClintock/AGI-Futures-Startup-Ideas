...
export function InteractiveScoreCard({
    ...
    const themeClass = "text-[var(--secondary)] border-[var(--primary)]/20 from-[var(--primary)]/10";

    const Icon = type === "moat" ? Shield : TrendingUp;

    return (
        <div className={`glass-panel p-6 sm:p-8 rounded-3xl border border-[var(--primary)]/20 relative overflow-hidden group mb-8`}>
            ...
            <div className="relative z-10">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
                    ...
                    <div className="flex items-baseline gap-1 relative z-20">
                        <span className={`text-4xl font-light tracking-tighter text-[var(--secondary)]`}>{score}</span>
                        <span className="text-white/40 font-mono text-sm">/ {maxScore}</span>
                    </div>
                </div>
                ...
                <button
                    ...
                    className={`relative z-20 mt-6 flex items-center text-sm font-mono uppercase tracking-widest text-[var(--secondary)] opacity-80 hover:opacity-100 transition-opacity cursor-pointer`}
                >
