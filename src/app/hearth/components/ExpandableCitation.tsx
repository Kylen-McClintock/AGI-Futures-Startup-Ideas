...
export function CitationSection({ citations }: { citations: CitationProps[] }) {
    return (
        <details className="mt-24 pt-12 border-t border-white/10 group">
            <summary className="text-sm font-mono uppercase tracking-widest text-white/70 mb-8 cursor-pointer list-none flex items-center gap-2 hover:text-white transition-colors">
                Acronyms & References
                <ChevronDown className="w-4 h-4 transition-transform group-open:rotate-180" />
            </summary>

            <div className="grid gap-4 sm:grid-cols-2 mt-4 pb-12">
                {citations.map((cite) => (
                    <div key={cite.number} className="flex gap-4 p-4 rounded-xl bg-white/5 border border-white/5 hover:border-[var(--primary)]/20 transition-colors">
                        ...
                        <div>
                            ...
                            {cite.url && (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="inline-flex items-center gap-1 text-xs font-mono text-white/40 hover:text-[var(--primary)] transition-colors"
                                >
