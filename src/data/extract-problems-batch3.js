...
const mdPath = '/Users/kylenmcclintock/Downloads/deep-research-report (4).md';
...
// Regex to match problem blocks. Adding support for lines like 21. ...
...
while ((match = problemRegex.exec(mdContent)) !== null) {
    const parsedRank = parseInt(match[1]);
    ...
    // Split concatenated sources like "[1] ... [2] ..." into multiple list items
    let finalSources = [];
    ...
    // The document ranks are 21-40, we want them to map to 41-60. Diff is +20 from document rank.
    newProblems.push({
        rank: parsedRank + 20, 
        slug,
        title,
...
console.log(`Parsed ${newProblems.length} new problems for Batch 3`);
...
if (arrayEndIndex !== -1) {
    ...
    const beforeEnd = tsContent.slice(0, arrayEndIndex - 1); // remove newline before target 
    const newTsContent = beforeEnd + ',\n    ' + jsonStr + '\n' + tsContent.slice(arrayEndIndex);
    fs.writeFileSync(tsPath, newTsContent);
    console.log('Successfully injected Batch 3 into problem-atlas-data.ts');
} else {
